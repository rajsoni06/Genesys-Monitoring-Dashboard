...
app.post("/proxy/sales-get-customer-by-cof", async (req, res) => {
  try {
    const { cof } = req.body;
    // console.log("Proxy: Received request for /proxy/sales-get-customer-by-cof with COF:", cof);
    if (!cof) {
      console.error("Proxy: Missing COF in request body.");
    ...
    const text = await response.text();
    // console.log("Proxy: Raw response from external Sales_GetCustomerbyCOF API:", text);
    let data;
    try {
      data = JSON.parse(text);
    } catch (e) {
      console.error("Proxy: Failed to parse external Sales_GetCustomerbyCOF API response as JSON:", e);
    ...
    if (!response.ok) {
      console.error("Proxy: External Sales_GetCustomerbyCOF API returned non-OK status:", response.status, data);
      return res.status(response.status).json({
        error: `API request failed: ${response.status}`,
        details: data,
      });
    }
    // console.log("Proxy: External Sales_GetCustomerbyCOF API response data:", data);
    res.json(data);
  } catch (err) {
